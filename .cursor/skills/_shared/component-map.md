...
| file | `app/components/file.js` | `controls-file.css` | — | `download`, `upload`, `remove-circle`, `clear` | `dom`, `icons` | Segmented rows + `.file--large` + `.file--fullscreen`; optional upload/remove; hover/always/never ext+size |
