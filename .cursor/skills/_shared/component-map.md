...
| model-preview | `app/components/model-preview.js` | `controls-model.css` | `app/vendor/three/` | `home`, `fullscreen`, `play`, `pause` | `config`, `dom`, `orbit-home`, `icons`; maximise: expandable-surface | Interactive Three.js host for the shared indexed mesh shape; pages need a `three` import map; optional meta strip, home/reset, play/pause auto-rotate, and maximise |
