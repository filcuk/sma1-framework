...
| **Light** | Lighter background only (no accent selection border) | Not joined as a selection run | Muted segmented (`.segmented-control--muted`), including the footer theme switch |
...
**Light** recipe: selected / pressed state uses a softer fill only (today: `background: var(--code-bg)` on `.segmented-control--muted`), without the accent selection border used by standard. Muted segments still use the standard accent hover border and exit animation.
